...
class QuickViewLog:    
    def signalLOGAnalysis(self):
        ...
        for line in f.readlines():
            ...
            if "STATE:: == EN ==>" in line:
                ...
                self.stateResult = self.stateResult + "[" + spl[1] + "] --> " + spl[5] + "\n"
                self.result =  self.result + "[" + spl[1] + "] --> " + spl[5] + "\n"
            elif "ERROR_" in line:
                line = re.sub(u"([^\u0030-\u0039 . _ \u0041-\u005a])", "", line)
                spl = [errorDict[x] if x in errorDict else x for x in line.split( )]
                self.errorResult = self.errorResult + "[" + spl[1] + "] --> " + spl[5] + "\n"
                self.result = self.result + "[" + spl[1] + "] --> " + spl[5] + "\n"
            elif "AUDIO" in line:
                line = re.sub(u"([^\u0030-\u0039 . _ \u0041-\u005a])", "", line)
                spl = line.split( )
                self.audioResult = self.audioResult + "[" + spl[1] + "] --> " + spl[4] + "\n"
                self.result = self.result + "[" + spl[1] + "] --> " + spl[4] + "\n"
            elif "USR_CTR" in line:
                line = re.sub(u"([^\u0030-\u0039 . _ \u0041-\u005a])", "", line)
                spl = [ctrDict[x] if x in ctrDict else x for x in line.split( )]
                self.ctrResult = self.ctrResult + "[" + spl[1] + "] --> " + spl[4] + "\n"
                self.result = self.result + "[" + spl[1] + "] --> " + spl[4] + "\n"
    ...
    def SFMStateAnalysis(self):
        self.text.delete(0.0, END)
        self.text.insert(INSERT, self.stateResult)


    def SFMAudioAnalysis(self):
        self.text.delete(0.0, END)
        self.text.insert(INSERT, self.audioResult)


    def SFMErrorAnalysis(self):
        self.text.delete(0.0, END)
        self.text.insert(INSERT, self.errorResult)


    def SFMCtrAnalysis(self):
        self.text.delete(0.0, END)
        self.text.insert(INSERT, self.ctrResult)
    ...
    def setupWindow(self):
        ...
        self.quickViewWin.geometry('200x200')
    ...
    def __init__(self, logFilePath):
        ...
        self.menubar.add_cascade(label = "状态机分析", menu = self.sfmMenu, font = self.font)

        for item in SFMList:
            if item == "整体分析":
                self.sfmMenu.add_command(label=item, command=self.SFMAnalysis, font=self.font)
            elif item == "状态分析":
                self.sfmMenu.add_command(label=item, command=self.SFMStateAnalysis, font=self.font)
            elif item == "错误分析":
                self.sfmMenu.add_command(label=item, command=self.SFMErrorAnalysis, font=self.font)
            elif item == "语音分析":
                self.sfmMenu.add_command(label=item, command=self.SFMAudioAnalysis, font=self.font)
            elif item == "按键分析":
                self.sfmMenu.add_command(label=item, command=self.SFMCtrAnalysis, font=self.font)
            else:
